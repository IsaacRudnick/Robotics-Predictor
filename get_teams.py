...
def get_team_matches(team_key):
    ...
    event_keys = get_api_data(f"/team/{team_key}/events/{YEAR}/keys")
    ...
    for event_key in event_keys:
        ...
        path = f'data/events/{event_key}.json'.replace(YEAR, '')
...
if __name__ == "__main__":
    ...
    teams = []
    for i in range(20):  # Each page has 500 teams, so we overshoot and ask for 20 pages (10k teams, which is more than enough)
        teams += get_api_data(f"/teams/{YEAR}/{i}/keys")
